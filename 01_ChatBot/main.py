...
app = FastAPI(lifespan=lifespan) # 与app.on_event()方法等价
...
async def chat_completions(request: ChatCompletionRequest):
    ...
    try:
        ...
        # 处理非流式响应处理
            ...
            try:
                events = graph.stream({"messages": prompt}, config) # events是一个生成器，每次生成一个结果
                for event in events: # event结构是{"node_name":state}，每经过一个节点，就生成一个结果
                    logger.info(f"结果: {event}")
                    for value in event.values(): # 只关心state
                        result = value["messages"][-1].content # 取最新内容
            except Exception as e:
