...
def graph_response(graph: StateGraph, user_input: str, config: dict, tool_config: ToolConfig) -> None:
    ...
    try:
        ...
        # 遍历事件流
        for event in events: # 一个event是一个node，events是个生成器，其内包含node的产出，非全部state。event是个dict。
            logger.info(f"Event: {event}")
            # 遍历事件中的值
            for value in event.values(): # 一个value是一个node的状态，即该node的产出，而非全部state
...
def main():
    ...
    try:
        ...
        # 定义运行时配置，包含线程ID和用户ID
        config = {"configurable": {"thread_id": "127", "user_id": "123"}}
        # 进入主循环
        while True:
            # 获取用户输入并去除首尾空格
